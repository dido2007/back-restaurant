...
module.exports = (db) => {
  ...
  function generateVerificationCode() {
   ...
   function hasCrudPermissions(role) {
      return role === 'admin' || role === 'dev' || role === 'serveur' || role === 'client';
   }
   ...
   router.post("/forgot-password", async (req, res) => {
      ...
      if (!user) {
          ...
          await db.collection("login").updateOne({ email }, { $set: { verificationCode } });
      ...
      router.post("/reset-password", async (req, res) => {
         ...
         if (!user) {
             ...
             await db.collection("login").updateOne({ email }, { $set: { password: hashedPassword }, $unset: { verificationCode: "" } });
             res.json({ error: "" });
         }
     });
